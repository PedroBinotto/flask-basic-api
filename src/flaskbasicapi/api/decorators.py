...
from flaskbasicapi.api.app import db
from flaskbasicapi.api.schemas import StringPaginationSchema, PaginatedCollection
    ...
    def inner(f):
        ...
        def paginate(*args, **kwargs):
            ...
            count = db.session.scalar(
                sqla.select(sqla.func.count()).select_from(select_query)
            )
            ...
            if after is not None:
                ...
                offset = db.session.scalar(
                    sqla.select(sqla.func.count()).select_from(
                        select_query.filter(offset_condition)
                    )
                )
            else:
                if offset is None:
